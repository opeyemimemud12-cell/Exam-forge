...
const ExamSession = {
  ...
  submit(reason = 'manual') {
    ...
    const maxScore  = parseInt(exam.maxScore)  || 100;
    const passScore = parseInt(exam.passScore) || 50;
    const score   = Math.round((correct / exam.questions.length) * maxScore);
  ...
  _showResults(score, percent, grade, passed, remark) {
    ...
    document.getElementById('res-score').textContent = `${score}/${parseInt(exam.maxScore)||100}`;
