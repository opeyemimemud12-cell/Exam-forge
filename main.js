...
function importExam(e) {
  ...
  r.onload = ev => {
    try {
      ...
      if (!data.title || !Array.isArray(data.questions)) throw new Error('Bad format');
      delete data.id; data.taken=false; data.lastResult=null; data.takenAt=null;
      DB.add(data); renderExamsList();
      showToast('📥 Exam imported!','success'); showApp('exams');
    } catch { showToast('❌ Invalid exam file','error'); }
  };
